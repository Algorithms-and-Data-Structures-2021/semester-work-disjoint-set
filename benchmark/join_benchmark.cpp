...
int main(int argc, char **argv) {
  ...
  if (argc != 4) {
    ...
    for (int i = 1; i <= num_datasets; i++) {
      ...
      // Делаем на <num_trials> меньше операций <join>,
      // т.к. на N элементов можно сделать максимум n-1 объединения,
      // а следующие объединения не будут иметь смысла
      ...
      // Замеряем время операции объединения 2 случайных элементов
