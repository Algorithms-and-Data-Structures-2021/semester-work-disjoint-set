...
int main(int argc, char **argv) {
  ...
  if (argc != 4) {
    ...
    for (int i = 1; i <= num_datasets; i++) {
      // Открываем файлы
      if (i < 10) {
        output_file = ofstream(path_project + "/benchmark/test_results/join_multiple/" + str_num_elem + "/0" + to_string(i) + ".csv");
        join_dataset = ifstream(path + "/join/0" + to_string(i) + "/" + str_num_elem + ".csv");
      } else {
        output_file = ofstream(path_project + "/benchmark/test_results/join_multiple/" + str_num_elem + "/" + to_string(i) + ".csv");
      ...
      // Замеряем время выполнения <num_elem> операций объединения
      ...
      for (int j = 0; j < num_trials; j++) {
        ...
        // Строим структуру данных, замеряем каждый <join>
        ...
        // Выводим результат в мс, удаляем структуру данных,
        // переводим "курсор" в <join_dataset> на начало файла
        result = result/1000000;
        output_file << result << endl;
        average += result;
        delete d_set;
        join_dataset.clear();
        join_dataset.seekg(0);
      }

      // Выводим среднее значение, готовимся к следующему набору данных
